...
public class ImageController {
    ...
    //The logic is to get the image from the databse with corresponding title. After getting the image from the
    // database the details are shown
    //First receive the dynamic parameter in the incoming request URL in a string variable 'title' and also
    // the Model type object
    ...
    //This controller method is called when the request pattern is of type 'images/upload' and also the
    // incoming request is of POST type
    //The method receives all the details of the image to be stored in the database, and now the image will be
    // sent to the business logic to be persisted in the database
...
//        List<Comment> imageComments =
    ...
    public String createComment(@PathVariable("imageId") Integer id, @RequestParam("comment") String comment, HttpSession http, Model model) {
        Image image = imageService.getImageById(id);
        User user = (User) http.getAttribute("loggeduser");
        ...
        commentService.uploadComment(comments);

        return "redirect:/images/" + id + "/" + image.getTitle();
    }

    //This controller method is called when the request pattern is of type 'editImage'
    //This method fetches the image with the corresponding id from the database and adds it to the model
    // with the key as 'image'
    //The method then returns 'images/edit.html' file wherein you fill all the updated details of the image

    //The method first needs to convert the list of all the tags to a string containing all the tags separated
    // by a comma and then add this string in a Model type object
    ...
    //The method adds the new imageFile to the updated image if user updates the imageFile and
    // adds the previous imageFile to the new updated image if user does not choose to update the imageFile
    ...
    //The method converts the string to a list of all the tags using findOrCreateTags() method and sets
    // the tags attribute of an image as a list of all the tags
    ...
    //findOrCreateTags() method has been implemented, which returns the list of tags after
    // converting the ‘tags’ string to a list of all the tags and also stores the tags in the
    // database if they do not exist in the database. Observe the method and complete the
    // code where required for this method.

    //Try to get the tag from the database using getTagByName() method. If tag is returned, you
    // need not to store that tag in the database, and if null is returned, you need to first store
    // that tag in the database and then the tag is added to a list
